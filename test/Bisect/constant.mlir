...
// CHECK-NEXT: [[CST0:%[a-z0-9_]+]] = arith.constant
// CHECK-NEXT: [[CST1:%[a-z0-9_]+]] = arith.constant
// CHECK-NEXT: [[R0:%[0-9]+]] = "test.A"([[ARG0]], [[CST0]]) : (tensor<f32>, tensor<f32>) -> tensor<f32>
// CHECK-NEXT: [[R1:%[0-9]+]] = "test.B"([[ARG0]]) {status = "culprit", which = 0 : i64} : (tensor<f32>) -> tensor<f32>
// CHECK-NEXT: [[R2:%[0-9]+]] = "test.C"([[R0:%[0-9]+]], [[R1]], [[CST1]]) {status = "failed", which = 0 : i64} : (tensor<f32>, tensor<f32>, tensor<f32>) -> tensor<f32>
...
func.func @simple(%arg0: tensor<f32>) -> tensor<f32> {
  ...
  %0 = "test.A"(%arg0, %cst0) : (tensor<f32>, tensor<f32>) -> tensor<f32>
  %1 = "test.B"(%arg0) : (tensor<f32>) -> tensor<f32>
  %2 = "test.C"(%0, %1, %cst1) : (tensor<f32>, tensor<f32>, tensor<f32>) -> tensor<f32>
